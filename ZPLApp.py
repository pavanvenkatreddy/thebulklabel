...
import queue
import os
from fuzzywuzzy import process  # Import the missing process module
            ...
            if score >= 80:
                ...
                if not result.empty:
                    ...
                    if sale_price and sale_price != item_price and float(sale_price) > 0:
                        zpl_code = self.generate_zpl_with_sale(product_name, item_price, sale_price, size_name)
                    else:
                        zpl_code = self.generate_zpl_without_sale(product_name, item_price, size_name)
